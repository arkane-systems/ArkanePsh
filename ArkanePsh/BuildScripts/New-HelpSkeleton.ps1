param ([string]$SnapinPath = $(throw "Path to module dll is required"), 
	   [string[]]$cmdletClassName = $(throw "Name of cmdlet class is required"), 
	   $OutputPath)
		...
		{	
			...
			$_.GetCustomAttributes($_CmdletAttribute, $false) | foreach `
			{
				...
				$cmdlet.GetCustomAttributes($false) |% {
					...
					if($type -eq 'ArkaneSystems.ArkanePsh.Annotations.DetailedDescriptionAttribute') 
					{
						$detailed = $_.Text
					}					
				}
